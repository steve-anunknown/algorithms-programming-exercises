...
bool exists(const unsigned int& el, const vector<unsigned int>& arr)
{
    ...
    return (it != arr.end() && *it == el);
}

/// @brief insert element into vector in the sorted position.
/// @param el the element to be inserted.
/// @param arr the vector which it is inserted into.
void insert(unsigned int el, vector<unsigned int>& arr)
{
    arr.insert(upper_bound(arr.begin(), arr.end(), el), el);
}
